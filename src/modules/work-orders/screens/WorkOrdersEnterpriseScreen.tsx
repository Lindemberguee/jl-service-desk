...
export default function WorkOrdersEnterpriseScreen() {
  ...
  const { search, setSearch, status, setStatus, priority, setPriority, filtered } = useWorkOrderFilters(tabFiltered as WorkOrder[]);
  ...
  return (
    <div className="space-y-5">
      ...
      <WorkOrdersKpiStrip
        items={[
          { label: 'Total de OS', value: tabCounts.all, helper: 'Volume operacional atual', tone: 'default' },
          { label: 'Em andamento', value: tabCounts.in_progress, helper: 'Atendimentos em execução', tone: 'warning' },
          { label: 'Atrasadas', value: overdueCount, helper: 'Demandas fora do SLA', tone: overdueCount > 0 ? 'danger' : 'success' },
          { label: 'SLA dentro do prazo', value: `${withinSlaRate}%`, helper: 'Saúde operacional consolidada', tone: withinSlaRate >= 90 ? 'success' : withinSlaRate >= 75 ? 'warning' : 'danger' },
        ]}
      />
        ...
        <div className="flex items-center gap-2 overflow-x-auto pb-1">
          {tabs.map((tab) => (
            <button
              ...
              className={`inline-flex items-center gap-2 rounded-lg border px-3 py-2 text-sm font-medium transition-all ${activeTab === tab.key ? 'border-primary bg-primary/10 text-primary shadow-sm' : 'border-transparent text-muted-foreground hover:border-border hover:bg-muted/40 hover:text-foreground'}`}
            >
              {tab.label}
              <span className={`rounded-md px-1.5 py-0.5 text-[11px] font-semibold ${activeTab === tab.key ? 'bg-primary/15 text-primary' : 'bg-muted text-muted-foreground'}`}>
                {tab.count}
              </span>
            </button>
          ))}
      ...
      {isLoading ? (
        ...
        <div className="rounded-xl border border-border bg-card p-3 shadow-sm">
          <WorkOrdersTable data={filtered} onRowClick={(wo) => navigate(`/os/${wo.id}`)} />
        </div>
      )}
