...
export function getLinkedKpiIds(kr: Pick<KeyResultWithLinks, 'kpi_id'> & Partial<Pick<KeyResultWithLinks, 'kpi_ids'>>) {
  const ids = [...(kr.kpi_ids || []), ...(kr.kpi_id ? [kr.kpi_id] : [])].filter(Boolean) as string[];
  return Array.from(new Set(ids));
}

export function getKrCompletionRatio(kr: Pick<KeyResultWithLinks, 'start_value' | 'target_value' | 'current_value' | 'activity_status'>) {
...
export function getAutoKpiValue(kpi: Pick<Kpi, 'id' | 'target_value'>, keyResults: KeyResultWithLinks[]) {
  ...
  const completedCount = linkedKeyResults.filter((kr) => COMPLETED_ACTIVITY_STATUSES.has(kr.activity_status)).length;
...
export function getEffectiveKpiValue(kpi: Pick<Kpi, 'id' | 'target_value'>, entries: KpiEntry[], keyResults: KeyResultWithLinks[]) {
