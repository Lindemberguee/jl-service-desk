...
import { ClipboardList, LayoutDashboard, User, LogOut, Sun, Moon, Wrench } from 'lucide-react';
...
const navItems = [
  ...
  { label: 'Minhas OS', icon: ClipboardList, path: '/tech/os' },
  { label: 'Perfil', icon: User, path: '/tech/perfil' },
];
  ...
  return (
    <div className="min-h-screen flex flex-col bg-background">
      ...
      <header className="h-14 border-b border-border bg-card flex items-center px-4 gap-3 shrink-0">
        <div className="flex items-center gap-2.5 cursor-pointer" onClick={() => navigate('/tech')}>
          {tenant?.logo_url ? (
            <img src={tenant.logo_url} alt={tenant.name} className="h-8 w-8 rounded-lg object-cover" />
          ) : (
            <div className="h-8 w-8 rounded-lg flex items-center justify-center" style={{ backgroundColor: tenant?.primary_color || 'hsl(var(--primary))' }}>
              <Wrench className="h-4 w-4 text-primary-foreground" />
            </div>
          )}
          <div className="hidden sm:block">
            <p className="text-sm font-bold leading-none">{tenant?.name || 'ServiceOS'}</p>
            <p className="text-[11px] text-muted-foreground">Painel Técnico</p>
          </div>
        </div>
        ...
        <span className="text-xs text-muted-foreground hidden sm:block truncate max-w-[120px]">{profile?.name}</span>
      ...
      <div className="md:hidden fixed bottom-0 left-0 right-0 border-t border-border bg-card z-50 flex">
        {navItems.map(item => (
          <button
            key={item.path}
            className={`flex-1 flex flex-col items-center gap-0.5 py-2 text-[10px] transition-colors ${isActive(item.path) ? 'text-primary' : 'text-muted-foreground'}`}
            onClick={() => navigate(item.path)}
          >
